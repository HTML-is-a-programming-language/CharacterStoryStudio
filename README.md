...
**Phase 2 — AI 스토리 생성 파이프라인(Mock Provider) 완료**

- Phase 0: 프로젝트 설계 및 기초 문서 작성 완료
- Phase 1: 고정 JSON → Remotion MP4 렌더링 POC 완료
- Phase 2: 대화(`sample-conversation.json`)를 분석해 감정적으로 의미 있는 장면을 추출하고, 서로 다른 톤의 컨셉 3안을 제시한 뒤, 그중 하나를 5씬 스토리보드로 변환하는 `StoryProvider`/`MockStoryProvider` 파이프라인을 구현했습니다. 생성된 스토리보드는 Phase 1과 같은 Remotion 컴포지션으로 렌더링되고, `ffprobe` 기반 QA 스크립트로 해상도/재생시간을 자동 검증합니다.
