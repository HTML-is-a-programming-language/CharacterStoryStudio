...
- Phase 7: 모든 씬이 승인되면 `/story/[conceptId]/render`에서 실제로 Remotion 렌더링을 실행해 MP4를 다운로드할 수 있습니다. HTTP 요청으로 실제 렌더링→다운로드→QA까지 성공을 확인했습니다(요청당 약 40~50초 소요, 동기 처리·큐 없음).
