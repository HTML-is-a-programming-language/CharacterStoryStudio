...
- Phase 1: 원작 5씬 샘플 스토리 "책갈피"를 Zod로 검증되는 고정 JSON으로 작성하고, Remotion으로 9:16 세로형 MP4까지 렌더링하는 파이프라인을 검증했습니다. 이미지 생성 API 없이 그라디언트 배경 + 타이포그래피 애니메이션만으로 모션코믹의 뼈대(장면 전환, 대사 캡션)를 구현했습니다.
...
pnpm run render   # out/story.mp4로 렌더링
pnpm run test     # 스키마 검증 테스트
pnpm run typecheck
```
