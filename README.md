...
**Phase 12 — 렌더링 비동기화(인메모리 잡 큐) 완료, 이후 렌더링 버그 수정 + QA 자동 통합 + Playwright E2E 도입**
...
- QA 자동 통합: 해상도/재생시간을 검사하던 `pnpm run qa` CLI 스크립트를 렌더링 파이프라인에 연결했습니다. 이제 렌더링이 끝나면 자동으로 QA가 돌고, 결과가 다운로드 링크와 함께 화면에 표시됩니다(QA 실패가 다운로드를 막지는 않습니다 — 정보성 표시입니다). 오디오 체크는 "스트림 존재"만으로는 부족해서(Remotion이 무음이어도 스트림 자체는 채워 넣음) `ffmpeg`의 `silencedetect`로 실제 무음 여부까지 검사하도록 확장했습니다.
- Playwright E2E 도입: 채팅→분석→컨셉 선택→씬 승인→렌더링→다운로드로 이어지는 핵심 흐름 전체를 실제 Chromium으로 검증하는 테스트(`e2e/story-flow.spec.ts`)를 추가했습니다. Mock Provider만 쓰므로 비용 없이 동작합니다.
...
pnpm run test:e2e                # Playwright로 채팅→렌더링→다운로드 전체 흐름 검증(dev 서버 자동 실행)
