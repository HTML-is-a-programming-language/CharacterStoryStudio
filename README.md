...
**Phase 5 — 이미지 Provider(Mock) 연동 완료**
...
- Phase 5: `ImageProvider` 추상화 + `MockImageProvider`를 추가했습니다. API 키 없이, 씬 정보를 시드로 한 결정론적 절차적 SVG(그라디언트+추상 도형)를 씬 이미지로 생성합니다. Next UI(`SceneCard`)와 Remotion(`StoryComposition`)이 같은 `sceneBackgroundStyle` 헬퍼로 이미지를 렌더링하고, "연출 재생성"을 누르면 이미지도 함께 바뀝니다. Phase 1의 손으로 쓴 고정 샘플은 이미지 없이 그라디언트 그대로 유지됩니다.
