...
**Phase 6 — 실제 이미지 Provider(OpenAI) 연동 완료**
...
- Phase 6: 실제 이미지 생성 API(OpenAI `gpt-image-1`) 연동. `IMAGE_PROVIDER=real` + `OPENAI_API_KEY`가 **둘 다** 있어야 켜지고(이중 opt-in), 하나라도 없으면 항상 Mock으로 동작합니다(Demo Mode 보장). **실제 API 키로 라이브 호출을 검증한 적은 없습니다** — 사용법은 아래와 `.env.example` 참고.
