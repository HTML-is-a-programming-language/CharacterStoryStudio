...
- Phase 8: `/chat`에서 캐릭터와 채팅하는 데모 화면을 추가했습니다. 실제 LLM 없이 고정 대본을 재생하는 Mock이며(입력 내용을 이해하지 않음), 이 프로젝트의 첫 Server Action을 도입했습니다. 대화가 끝나면 기존 컨셉 분석 화면(홈)으로 이어집니다.
