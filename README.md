...
- Phase 3: Next.js(App Router)를 기존 Remotion 프로젝트와 한 패키지에 공존시켜, 샘플 대화의 컨셉 3안을 카드로 보여주고 하나를 선택하면 5씬 스토리보드를 보여주는 최소 UI를 구현했습니다.
- Phase 4: 스토리보드 화면에 씬별 "승인"/"연출 재생성" 버튼을 추가했습니다. 대사(원본 대화 사실)는 재생성해도 바뀌지 않고 배경 연출만 바뀝니다. 승인/재생성 상태는 DB 없이 전부 URL 쿼리로 표현되어 새로고침·공유해도 그대로 재현됩니다. UI는 여전히 `MockStoryProvider`를 직접 호출하지 않고 `server-only`로 격리된 `sampleConversationPipeline.ts`를 통해서만 접근합니다.
