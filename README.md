...
**Phase 3 — 컨셉 선택 최소 UI(Next.js) 완료**
...
- Phase 3: Next.js(App Router)를 기존 Remotion 프로젝트와 한 패키지에 공존시켜, 샘플 대화의 컨셉 3안을 카드로 보여주고 하나를 선택하면 5씬 스토리보드를 보여주는 최소 UI를 구현했습니다. UI는 `MockStoryProvider`를 직접 호출하지 않고 `server-only`로 격리된 `sampleConversationPipeline.ts`를 통해서만 접근합니다.
