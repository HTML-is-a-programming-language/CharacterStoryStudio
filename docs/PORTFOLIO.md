...
## QA 체크 항목 확장 — "스트림 존재"에서 "실제 무음 여부"로

### 무엇을 했나

1. **QA 통합 직후 "다음 권장 작업"으로 QA 체크 확장을 선택받음**: 오디오 스트림 존재 여부를 새 체크로 먼저 추가했다 — 비디오 프레임을 샘플링해서 완전 검은색이 아닌지 확인하는 건 복잡도가 더 커서 이번 범위에서 뺐다.
2. **실측해보니 방금 추가한 체크가 무의미했다**: `out/story.mp4`(오디오 데이터가 아예 없는 Phase 1 고정 샘플)로 테스트하자 "오디오 스트림 존재"가 **통과**해버렸다. Remotion의 `renderMedia`가 `<Audio>`가 하나도 없어도 무음 오디오 스트림을 컨테이너에 채워 넣기 때문이었다 — `ffmpeg`로 직접 까보니 스트림은 있지만 28초 전체가 완전 무음이었고, 정확히 이번에 고친 버그(ADR-021)와 같은 증상이었다. 이 사실을 확인 즉시 사용자에게 알리고("이 체크는 실제로 겪은 버그를 못 잡습니다"), 어떻게 할지 다시 물었다.
3. **실제 무음 감지로 확장**: 사용자가 선택한 대로 `@ffmpeg-installer/ffmpeg`를 새 의존성으로 추가하고, `silencedetect` 필터로 재생시간 대비 무음 구간 길이를 재는 "오디오 무음 아님" 체크를 만들었다. Remotion이 내부적으로 쓰는 ffmpeg 바이너리(이번 세션에서 `silencedetect`는 되고 `volumedetect`는 안 되는 걸 이미 확인했었다)는 공개 API가 아니라서 의존하지 않고, `@ffprobe-installer/ffprobe`와 같은 패턴으로 별도 패키지를 썼다.
4. **같은 웹팩 문제를 이번엔 예방적으로 막았다**: `@ffprobe-installer/ffprobe`가 냈던 것과 같은 종류의 웹팩 번들링 문제가 예상되어, 실제로 빌드를 돌려보기 전에 `next.config.mjs`의 `serverComponentsExternalPackages`에 `@ffmpeg-installer/ffmpeg`를 먼저 추가해뒀다 — 실제로 `next build`가 수정 없이 바로 통과했다.

### 실행 결과

```
pnpm run typecheck → 통과
pnpm run test        → 91 passed (qaCheck 6개로 보강)
pnpm exec next build → 성공 (예방적 조치 덕분에 웹팩 에러 없이 바로 통과)

실측 검증:
  pnpm run qa out/story.mp4     → ✗ 오디오 무음 아님 — 무음 28.05s / 전체 28.05s (exit 1)
  pnpm run qa out/generated.mp4 → ✓ 오디오 무음 아님 — 무음 0.00s / 전체 28.05s

dev 서버 + curl: 렌더링 시작→폴링(12회, ~36초)→완료 응답에 4개 체크
  (해상도/재생시간/오디오 스트림/오디오 무음 아님) 모두 정상 포함, 다운로드 200 확인
```

### 겪은 이슈

- **"그럴듯해 보이는 체크"를 실제 실패 케이스로 검증하지 않았다면 잘못 보고할 뻔했다**: 오디오 스트림 존재 체크를 추가하고 바로 "이걸로 이번 버그가 재발해도 잡힙니다"라고 보고하려다, `out/story.mp4`라는 알려진 실패 케이스가 이미 저장소에 있다는 걸 떠올려 먼저 돌려봤다. 그 결과 체크가 무의미하다는 걸 구현 직후에 알았다 — ADR-021의 mp3 MIME 가설 때와 같은 종류의 실수를 반복할 뻔했다는 걸 문서에도 남긴다. 검증 없이 통과했다고 보고하지 않는다는 원칙(CLAUDE.md)이 실제로 잘못된 보고를 막아준 사례다.

### 왜 이렇게 결정했나

근거는 [DECISIONS.md](./DECISIONS.md) ADR-022 하단(추가 항목) 참고.
